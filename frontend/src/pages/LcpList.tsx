...
const LcpList: React.FC = () => {
  ...
  const loadLcpItems = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/lcp?${params}`, {
        headers: {
  ...
  const handleDelete = async (item: LcpItem) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        await loadLcpItems();
        alert('✅ LCP item permanently deleted from database: ' + (data.message || 'Item deleted successfully'));
      } else {
        alert('❌ Failed to delete LCP item: ' + (data.message || 'Failed to delete item'));
      }
    } catch (error) {
      console.error('Error deleting LCP item:', error);
      alert('Failed to delete LCP item: ' + (error instanceof Error ? error.message : 'Unknown error'));
    } finally {
      setDeletingItems(prev => {
        const newSet = new Set(prev);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        closePanel();
        resetForm();
        alert(data.message || `LCP item ${editingItem ? 'updated' : 'added'} successfully`);
      } else {
        if (data.errors) {
          const errorMessages = Object.values(data.errors).flat().join('\n');
          alert('Validation errors:\n' + errorMessages);
        } else {
          alert(data.message || `Failed to ${editingItem ? 'update' : 'add'} LCP item`);
        }
      }
    } catch (error) {
      console.error('Error submitting form:', error);
      alert(`Failed to ${editingItem ? 'update' : 'add'} LCP item: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setSavingForm(false);
    }
  };
  ...
  const resetForm = () => {
    setFormData({
      name: '',
      modified_by: 'ravenampero0123@gmail.com',
      modified_date: new Date().toISOString().slice(0, 16).replace('T', ' ') + ' am'
    });
  };
  ...
  const renderTableRow = (item: LcpItem) => {
    return (
      ...
      <div className="bg-gray-900 border-b border-gray-800">
        <div className="px-6 py-4">
          <div className="flex items-center justify-between mb-4">
            <h1 className="text-xl font-semibold text-white">LCP</h1>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-800 border-b border-gray-700">
                <tr>
                  <th className="px-6 py-4 text-left text-white font-medium">LCP Name</th>
